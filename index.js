...
} = require("@aws-sdk/client-ecr"); // CommonJS import
...
const getFindings = async (ECR, repository, tag) => {
    ...
    try {
        let findings = await ECR.send(command)
        ...
        if (err.name === ScanNotFoundException.name) {
            return null
        }
        throw err
    }
...
const getAllFindings = async (ECR, repository, tag) => {
    return await getPaginatedResults(async (NextMarker) => {
        ...
        try {
            const findings = await ECR.send(command)
            return {
